...
export default function CountryPage({ country, packages = [] }: CountryPageProps) {
  const { t } = useLanguage();
  ...
  return (
    <div>
      ...
      <section className="py-20 text-center text-white relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #3B82F6 0%, #60A5FA 100%)' }}>
        <div className="absolute inset-0 w-full h-full bg-cover bg-center opacity-30 scale-110" style={{ backgroundImage: `url(${packages[0]?.image})` }}></div>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 relative">
          <h1 className="text-4xl md:text-5xl font-bold mb-4" style={{ textShadow: '0 2px 4px rgba(0,0,0,0.2)' }}>{t(country)}</h1>
          <p className="max-w-3xl mx-auto text-lg md:text-xl">{t(`${country}Desc`)}</p>
        </div>
      </section>
      ...
      <section className="py-20">
        <Container>
          <Row className="align-items-center mb-5">
            <Col lg={6}>
              <h2 className="text-3xl font-bold mb-4">{t(`discover${country.charAt(0).toUpperCase() + country.slice(1)}`)}</h2>
              <p className="text-gray-600 mb-4 text-lg">{t(`${country}About1`)}</p>
              <p className="text-gray-600 mb-4 text-lg">{t(`${country}About2`)}</p>
              <p className="text-gray-600 text-lg">{t(`${country}About3`)}</p>
            </Col>
            <Col lg={6}>
              ...
              <Col md={6} lg={3} key={index}>
                <Card className="h-100 border-0 shadow-lg overflow-hidden">
                  ...
                  <Card.Body>
                    ...
                    <Button variant="primary" className="w-100" href={getWhatsAppLink(`I'm interested in: ${t(pkg.nameKey)}`)} target="_blank">{t('bookNow')}</Button>
                  </Card.Body>
                </Card>
              </Col>
            ))}
